...
resource "null_resource" "run_script" {
  provisioner "remote-exec" {
    ...
    inline = [
      "echo 'hello from awsdevopsmulticloud new version 2.0' >> /home/ubuntu/file200"
    ]
  }
...
#Re-running null_resource depends on timestamp not on content modification.. becuse state file is not tracking inside the content 
